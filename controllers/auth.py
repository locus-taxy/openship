...
from schemas.auth import SignupRequest, LoginRequest, RefreshRequest, AuthResponse
...
def login_user(payload: LoginRequest, response: Response):
    ...
    response.set_cookie(
        ...
        samesite="strict", max_age=JWT_REFRESH_TOKEN_EXPIRE_DAYS * 86400,
        path="/auth/refresh",
    )
    return AuthResponse(user=_user_dict(user), access_token=access_token, refresh_token=refresh_token)
